...
pub fn parse_font_face_rule(
    ...
    let variable = weight_clean.split_whitespace().count() > 1 || style_clean.split_whitespace().count() > 1;
    let identity = font_identity_key(family, &weight_clean, &style_clean, unicode_clean.as_deref());
...
fn font_identity_key(family: &str, weight: &str, style: &str, unicode_range: Option<&str>) -> String {
    format!(
        "{}|{}|{}|{}",
