...
pub async fn download_fonts(
    ...
    emit_log(logger.as_ref(), format!("Downloading {} font variant(s)", fonts.len()));
    ...
    if allow_browser_fallback && !browser_retry.is_empty() {
        emit_log(
            logger.as_ref(),
            format!("Retrying {} download(s) in browser context", browser_retry.len()),
        );
...
fn failure_line(font: &FontCandidate, detail: impl Into<String>) -> String {
    format!("{} ({} {}): {}", font.family, font.weight, font.style, detail.into())
}
