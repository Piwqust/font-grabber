...
impl BrowserFetcher {
    pub async fn connect(webdriver_url: &str, page_url: &str, logger: Option<Logger>) -> Result<Self> {
    ...
    pub async fn fetch_font(&mut self, font: &FontCandidate, cache_dir: &Path) -> Result<CachedFont> {
        let mut last_error = None;

        for source in &font.sources {
            let result = self
                .driver
                .execute(BROWSER_FETCH_SCRIPT, vec![Value::String(source.url.clone())])
                .await;
