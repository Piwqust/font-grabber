...
use crate::{
    domain::{CachedFont, ConvertedFont, FontFormat, OutputFormat, VariableAxis},
...
pub fn convert_fonts(cached_fonts: &[CachedFont], logger: Option<Logger>) -> Result<ConversionBatch> {
    ...
    for (index, cached) in cached_fonts.iter().enumerate() {
        emit_log(
            logger.as_ref(),
            format!("Converting {}/{}: {}", index + 1, cached_fonts.len(), cached.info.family),
        );
    ...
    Ok(ConversionBatch { converted, failures })
}

fn convert_font(cached: &CachedFont) -> Result<ConvertedFont> {
    let raw = fs::read(&cached.cached_path)
        .with_context(|| format!("Failed to read cached font {}", cached.cached_path.display()))?;
...
fn decompress_to_sfnt(data: &[u8], format: FontFormat) -> Result<Vec<u8>> {
    match format {
        ...
        FontFormat::EmbeddedOpenType | FontFormat::Svg => bail!("Unsupported font format: {}", format.label()),
...
mod tests {
    ...
    fn detect_sfnt_type_distinguishes_ttf_and_otf() {
        assert_eq!(detect_sfnt_type(&0x0001_0000u32.to_be_bytes()).unwrap(), OutputFormat::Ttf);
