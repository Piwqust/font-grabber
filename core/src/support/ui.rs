...
pub fn print_grab_report(report: &GrabReport) {
    ...
    println!(
        "{} {}",
        style("Saved").dim(),
        style(report.saved_count).green()
    );
            ...
            println!(
                ...
                style(format!(
                    ...
                    variant_label(
                        &saved.weight,
                        &saved.style,
                        saved.stretch.as_deref(),
                        saved.variable_axes_preserved
                    )
                ))
                .bold(),
...
pub fn print_doctor_report(report: &DoctorReport) {
    ...
    println!(
        "{} {}",
        style("Status").dim(),
        status_text(report.status_ok)
    );
    println!(
        "{} {}",
        style("Session").dim(),
        status_text(report.session_ok)
    );
    println!(
        "{} {}",
        style("Script").dim(),
        status_text(report.script_ok)
    );
}
...
fn script_badge(font: &FontCandidate) -> String {
    if !font.scripts.is_empty() {
        font.scripts
            ...
            .collect::<Vec<_>>()
            .join(", ")
    } else if font.unicode_range.is_some() {
        "subset".to_string()
    } else {
        String::new()
    }
...
fn badge_suffix(font: &FontCandidate) -> String {
    let badge = script_badge(font);
    if badge.is_empty() {
        String::new()
    } else {
        format!(" — {badge}")
