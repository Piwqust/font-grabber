...
pub fn merge_fonts(
    existing: Vec<FontCandidate>,
    incoming: Vec<FontCandidate>,
) -> Vec<FontCandidate> {
    ...
    for font in existing.into_iter().chain(incoming) {
        ...
        let incoming_scripts = font.scripts.clone();
        ...
        match map.get_mut(&key) {
            Some(current) => {
                ...
                current.scripts.extend(incoming_scripts);
                current.scripts.sort();
                current.scripts.dedup();
...
pub(crate) fn font_identity_key(
    family: &str,
    weight: &str,
    style: &str,
    stretch: Option<&str>,
...
pub(crate) fn normalize_font_family(value: &str) -> String {
    let token = first_font_family_token(value.trim());
    strip_wrapping_css_quotes(token.trim()).to_string()
}

fn first_font_family_token(value: &str) -> &str {
    let value = strip_wrapping_css_quotes(value.trim());
    let mut quote = None;
...
fn strip_wrapping_css_quotes(mut value: &str) -> &str {
    loop {
        let trimmed = value.trim();
        let bytes = trimmed.as_bytes();
        let has_matching_quotes = bytes.len() >= 2
...
mod tests {
    ...
    #[test]
    fn parse_font_face_rule_keeps_only_primary_family_name() {
        let fonts = parse_font_face_rule(
            r#""'tttrailersregular', sans-serif""#,
            "normal",
            "400",
            None,
            r#"url("/fonts/TT_Trailers2_Regular.woff2") format("woff2")"#,
            None,
            "https://example.com/page",
            ScanSource::StaticCss,
        )
        ...
        assert_eq!(fonts[0].family, "tttrailersregular");
        ...
        let fonts = parse_font_face_rule(
            ...
            "400",
            None,
            r#"url("/fonts/TT_Commons.woff2") format("woff2")"#,
            None,
            "https://example.com/page",
            ScanSource::StaticCss,
        )
        ...
        assert_eq!(fonts[0].family, "TTCommons");
    }
    ...
    fn merge_fonts_combines_duplicates_and_prefers_browser_source() {
        ...
        let browser_font = FontCandidate {
            ...
            unicode_range: None,
            scripts: vec![crate::domain::FontScript::Cyrillic],
            variable: true,
        ...
        assert_eq!(font.scripts, vec![crate::domain::FontScript::Cyrillic]);
    }
}
